...
// Estructura: { Provincia: { Cantón: { Distrito: 'XXXXX' } } }
export const CR_POSTAL = {
  'San José': {
    'San José (Central)': {
      'Carmen': '10101',
      'Merced': '10102',
...
export const PROVINCIAS = Object.keys(CR_POSTAL)

export function getcantones(provincia) {
  if (!provincia) return []
  return Object.keys(CR_POSTAL[provincia] || {})
}

export function getDistritos(provincia, canton) {
  if (!provincia || !canton) return []
  return Object.keys(CR_POSTAL[provincia]?.[canton] || {})
}

export function getCodigoPostal(provincia, canton, distrito) {
  return CR_POSTAL[provincia]?.[canton]?.[distrito] || ''
}
