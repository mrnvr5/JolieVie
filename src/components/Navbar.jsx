...
const navLinks = [
  { label: 'Libretas',              path: '/libretas' },
  { label: 'Repuestos',             path: '/repuestos' },
  { label: 'Accesorios',            path: '/accesorios' },
  { label: 'Preguntas frecuentes',  path: '/faq' },
  { label: 'Agenda tu llamada',     path: '/agenda' },
  { label: 'Sobre nosotras',        path: '/nosotras' },
]
...
function IconBag({ count }) {
  return (
    <span className="relative">
      ...
      {count > 0 && (
        <span className="absolute -top-2 -right-2 bg-dark-red text-cream text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-alegreya-sc font-bold leading-none">
          {count}
        </span>
      )}
    </span>
...
export default function Navbar({ onSearchOpen }) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-cream border-b border-blush/30 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between h-16 md:h-20">
        ...
        <div className="hidden md:flex items-center gap-7">
          {navLinks.map(({ label, path }) => (
            <Link
              key={path}
              to={path}
              className="font-alegreya-sc text-brown/70 hover:text-dark-red transition-colors text-sm tracking-wide whitespace-nowrap"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-5 text-brown/60">
          ...
          <button
            ...
            className="md:hidden hover:text-dark-red transition-colors"
            aria-label="Menú"
          >
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-cream border-t border-blush/20 px-6 py-6 flex flex-col gap-5 animate-fade-in">
          {navLinks.map(({ label, path }) => (
            <Link
              key={path}
              to={path}
              onClick={() => setMenuOpen(false)}
              className="font-alegreya-sc text-brown/70 hover:text-dark-red transition-colors text-base tracking-wide"
            >
              {label}
            </Link>
          ))}
